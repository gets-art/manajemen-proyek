...
class TaskStatsOverview extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make(__('app.widgets.final_total'), number_format($task->final_total, 2) . ' IDR')
                ->description(__('app.widgets.project_value'))
                ->descriptionIcon('heroicon-m-banknotes')
                ->chart([7, 3, 4, 5, 6, 3, 5])
                ->color('primary'),

            Stat::make(__('app.widgets.paid_total'), number_format($task->paid_total, 2) . ' IDR')
                ->description(__('app.widgets.amount_received'))
                ->descriptionIcon('heroicon-m-check-circle')
                ->chart([3, 5, 6, 7, 4, 5, 8])
                ->color('success'),

            Stat::make(__('app.widgets.remaining'), number_format($task->rest_total, 2) . ' IDR')
            ...
            Stat::make(__('app.widgets.workers'), $workersCount)
                ->description(number_format($workersPaid, 2) . ' IDR ' . __('app.columns.paid'))
                ->descriptionIcon('heroicon-m-wrench-screwdriver')
                ->chart([2, 4, 6, 5, 7, 4, 6])
                ->color('info'),

            Stat::make(__('app.widgets.purchases'), $purchasesCount)
                ->description(number_format($purchasesTotal, 2) . ' IDR ' . __('app.columns.total'))
