...
class SupplierStatsOverview extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make(__('app.widgets.purchases_total'), number_format($purchasesTotal, 2) . ' IDR')
                ->description(__('app.widgets.total_purchase_value'))
                ->descriptionIcon('heroicon-m-banknotes')
                ->chart([5, 7, 6, 8, 9, 7, 10])
                ->color('info'),

            Stat::make(__('app.widgets.payments_total'), number_format($paymentsTotal, 2) . ' IDR')
                ->description(__('app.widgets.amount_paid_supplier'))
                ->descriptionIcon('heroicon-m-currency-dollar')
                ->chart([3, 5, 4, 6, 7, 5, 8])
                ->color('success'),

            Stat::make(__('app.widgets.balance'), number_format($balance, 2) . ' IDR')
